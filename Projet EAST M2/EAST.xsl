...
.section { page-break-after: always; background: #FFFFEB; margin: 2em 0.5em; padding: 0.5em; <xsl:if test="$impression='non'">border: gray outset</xsl:if> }
	...
	<xsl:choose>
		<xsl:when test="/EAST/PREFERENCES/PAGE/@image">
			...
			.slide { page-break-after: overflow: auto; always; margin: 2em 0.5em; padding: 0.5em; <xsl:if test="$impression='non'">border: gray outset;</xsl:if> background-size: cover; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-image: url('<xsl:value-of select="normalize-space(/EAST/PREFERENCES/PAGE/@image)"/>');}
...
div#erreur {
    ...
    <xsl:template match="LOGO_GAUCHE">
        ...
        <xsl:choose>
            ...
            <xsl:when test="@url!=''">
                <a href="{@url}"><img align="left" alt="{@alt}" border="0" src="{@fichier}" title="{@alt}" width="{@largeur_SVG}%"/></a>
            </xsl:when>
	    
	   <xsl:otherwise>
                <img align="left" alt="{@alt}" src="{@fichier}" title="{@alt}" width="{@largeur_SVG}%"/>
            </xsl:otherwise>
        ...
        <xsl:choose>
            ...
            <xsl:when test="@url!=''">
                <a href="{@url}"><img align="right" alt="{@alt}" border="0" src="{@fichier}" title="{@alt}" width="{@largeur_SVG}%"/></a>
            </xsl:when>
	    
            <xsl:otherwise>
                <img align="right" alt="{@alt}" src="{@fichier}" title="{@alt}" width="{@largeur_SVG}%"/>
            </xsl:otherwise>
